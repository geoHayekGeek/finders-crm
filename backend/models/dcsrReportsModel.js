...
/**
 * Calculate company-wide sales-side agent breakdown for a specific date range.
 * This includes all sales-side users, grouped by team leader when available and
 * under an "Unassigned" group when no team relationship exists.
 * @param {Date|string} startDateInput
 * @param {Date|string} endDateInput
 * @returns {Promise<object>}
 */
async function calculateCompanyDCSRAgentBreakdownData(startDateInput, endDateInput) {
  ...
  try {
    ...
    const companyMembers = membersResult.rows.map((row) => {
      ...
      const teamLeaderId = normalized === 'team leader'
        ? row.id
      ...
      return {
        ...
        team_leader_name: normalized === 'team leader'
          ? row.name
          : row.assigned_team_leader_name || 'Unassigned',
        team_leader_code: normalized === 'team leader'
          ? row.user_code || null
    ...
    const buildCountLookup = (rows, keyField = 'agent_id', valueField = 'count') => {
    ...
    const agentBreakdown = companyMembers.map((member) => {
      ...
      return {
        ...
        is_team_leader: normalized === 'team leader',
...
module.exports = {
  ...
  calculateCompanyDCSRAgentBreakdownData,
  createDCSRReport,
  getAllDCSRReports,
  getDCSRReportById,
  updateDCSRReport,
